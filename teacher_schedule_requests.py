...
def formatted_message(teacher_data, weekday, col_name, current_week):
    message = ""
    for i in range(1, 7):
        num = str(i)
        subject = teacher_data[col_name][weekday][num]["subject"]
        type_sub = teacher_data[col_name][weekday][num]["type"]
        aud = teacher_data[col_name][weekday][num]["aud"]
        group = teacher_data[col_name][weekday][num]["group"]
...
def make_teacher_schedule_message(teacher, command):
    ...
    for teacher_schedule in file_data["teachers"]:
        if teacher_schedule["teacher"].lower() == teacher:
            teacher_data = teacher_schedule["timetable"]
            if not teacher_data:
                print("Ошибка группы")
                ...
                if command == "TOD":
                    ...
                    if tomorrow == 6:
                        ...
                        message = f'расписание {teacher_schedule["teacher"]} на завтра\n'
                        col_name = "even" if even_week else "odd"
                        if tomorrow == 0:
                            current_week += 1
                            col_name = "odd" if even_week else "even"
                        message += formatted_message(teacher_data, weekdays[tomorrow], col_name, current_week)
                    result.append(message)
                    ...
                    for i in range(6):
                        message = ""
                        if i == 0:
                            message += f'расписание {teacher_schedule["teacher"]} на эту неделю'
                        message += "\n\n" + weekdays[i]
                        message += formatted_message(teacher_data, weekdays[i], col_name, current_week)
                        result.append(message)
                    ...
                    for i in range(6):
                        message = ""
                        if i == 0:
                            message += f'расписание {teacher_schedule["teacher"]} на следующую неделю'
                        message += "\n\n" + weekdays[i]
                        message += formatted_message(teacher_data, weekdays[i], col_name, current_week+1)
                        result.append(message)
                else:
